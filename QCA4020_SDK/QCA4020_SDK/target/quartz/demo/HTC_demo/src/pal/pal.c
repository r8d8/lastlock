 ...
 *-----------------------------------------------------------------------*/
#if defined(CONFIG_CDB_PLATFORM)
...
void PAL_CONSOLE_PRINTF(const char *format, ...)
{
    char buf[200] = {0};
    int len;
    va_list args;
    ...
    if ((len == sizeof(buf)) || (len < 0)) {
        PAL_CONSOLE_WRITE_STRING_LITERAL("PAL_CONSOLE_PRINTF: skip large printf\r\n");
        return;
    }

    PAL_Console_Write(len, buf);
}
