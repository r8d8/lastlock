...
/**
   @brief Prototype for the function called to get the User EDL data for BLE and
          802.15.4.

   If 'GetUsrEDLFileData()' is implemented in the application, it will be
   called during the boot process. This function should then assign the Data and
   Length parameters per the EDL file to use. This provides a means for the
   application to dynamically set the NVMs that will be used on boot, typically
   based on an application defined OTP value.

   If this function is not implemented, it will default to use the
   "UsrEDLFileData" variable.

   @param[out] Data   Pointer to the buffer containing the User EDL data to use.
   @param[out] Length Length of the provided buffer.
*/
typedef void (*GetUsrEDLFileData_t)(const unsigned char **Data, unsigned int *Length);
